...
1. 以下のコマンドを実行して、すべてのPrebuildを一覧表示します：

```shell
daytona prebuilds list
```

このコマンドを実行すると、DaytonaはPrebuildのリストを表示します。プロジェクト設定、リンクされているブランチ、コミット間隔、トリガーファイル、およびビルド保持期間を確認できます。
...
Prebuilds設定を更新すると、Daytonaは成功メッセージを表示します。
