...
デプロイされると、Runnerが起動し[Daytona Server](/docs/usage/server)に接続し、その時点でステータスが`Running`としてマークされます。
...
単一のRunnerは複数の[Providers](/docs/configuration/providers)を使用でき、デプロイ時またはジョブを通じて後から設定することで、様々な環境に対する柔軟性を実現します。
