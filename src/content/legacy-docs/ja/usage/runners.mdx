...
Runnerがすべてのリクエストを送信し、Serverは応答のみを行います。Runnerは*Heartbeat*（ステータスを共有するため）や*Job Requests*（割り当てられたジョブをDaytona Serverに要求するため）などのリクエストを送信します。
...
<Tabs>
  <TabItem label="フォアグラウンド">```shell daytona runner serve ```</TabItem>
  <TabItem label="デーモン/バックグラウンド">
    ```shell daytona runner start ```
  </TabItem>
</Tabs>
...
Runnerを設定すると、DaytonaはRunner設定を更新し、以下のメッセージを表示します：
