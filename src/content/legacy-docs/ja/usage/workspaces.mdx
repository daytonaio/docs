...
Daytonaでワークスペースを作成することは、プロジェクト用の専用開発環境を確保するための簡単なプロセスです。

Daytonaは、ワークスペースを作成するための2つの主要な方法を提供しています：
...
4. Workspaceをデプロイするターゲットを選択します。ターゲットを選択すると、[ターゲット設定](/docs/ja/configuration/target-config)の入力が求められます。設定は選択したターゲットによって異なります。

ターゲットの選択と設定が完了すると、Daytonaは指定されたGitリポジトリをクローンし、その内容に基づいて新しいWorkspaceをセットアップします。ターミナルにはWorkspaceの作成プロセスを示す進行状況情報が表示されます。
...
2. Workspaceをデプロイするターゲットを選択します。ターゲットを選択すると、[ターゲット設定](/docs/ja/configuration/target-config)の入力が求められます。設定は選択したターゲットによって異なります。
...
<Aside type="tip">
または、Workspaceのシェルで次のコマンドを実行してパブリックURLを生成することもできます：
