...
Workspacesは、クラウドベースやオンプレミス環境などの適切な[Target](/docs/usage/targets)インフラストラクチャを使用してホストされます。これにより、あるプロジェクトでの変更が他のプロジェクトに意図せず影響を与えることがなくなり、より安定で信頼性の高いワークフローが促進されます。これは特に共同作業環境において有益で、すべてのチームメンバーが同じ設定で作業することを保証し、環境関連の問題が発生する可能性を減らします。
...
GitリポジトリからWorkspaceを作成すると、[接続されたGit Provider](/docs/configuration/git-providers)またはカスタムGitリポジトリURLを使用して新しい開発環境がセットアップされます。
...
<Aside type="tip">
`--no-ide`フラグを使用して、[IDE](/docs/usage/ide)でWorkspaceを自動的に開くことをスキップします。
...
- **リポジトリのリストから選択**
   ...
   [Git Providerを接続](/docs/configuration/git-providers)している場合、Workspaceと接続するために選択できるリポジトリのリストが表示されます。
...
<Aside type="tip">
   ...
   これには[Builder](/docs/usage/builders)の設定も含まれます。
</Aside>

<br />

4. WorkspaceをデプロイするTargetを選択します。Targetを選択すると、[Target設定](/docs/configuration/target-config)の入力を求められます。設定は選択されたTargetによって異なります。

Targetの選択と設定が完了すると、Daytonaは指定されたGitリポジトリをクローンし、その内容に基づいて新しいWorkspaceをセットアップします。ターミナルにはWorkspaceの作成プロセスを示す進行状況情報が表示されます。
...
2. WorkspaceをデプロイするTargetを選択します。Targetを選択すると、[Target設定](/docs/configuration/target-config)の入力を求められます。設定は選択されたTargetによって異なります。
...
<Aside type="tip">
コマンドラインフラグを使用してWorkspace作成時にProject Builderを設定します。詳細については**[Builders](/docs/usage/builders)**を参照してください。
</Aside>
...
このコマンドを実行すると、DaytonaはあなたのWorkspaceの詳細とともに一覧を表示します。Workspace名、接続されているリポジトリ、Workspaceのターゲット、およびそのステータスを確認できます。
...
Daytonaは、既存のWorkspaceをIDEで開くオプションを提供します。これにより、お好みの開発環境でWorkspaceに素早くアクセスし、作業することができます。Daytonaを使用すると、異なるWorkspace間を簡単に切り替えて、効率的なワークフローを維持できます。

1. 既存のWorkspaceを開くには、以下のコマンドを実行します：

```shell
daytona code
```

このコマンドを実行すると、DaytonaはWorkspaceの詳細とともにWorkspaceのリストを表示します。Workspace名、Workspaceの一意識別子、および接続されているリポジトリを確認できます。
...
2. 開きたいWorkspaceを選択します。

:::note
お好みのIDEを使用するようにDaytonaを設定してください。
詳細については、**[デフォルトIDEの設定](/docs/usage/ide#set-the-default-ide)**を参照してください。
:::
...
Daytonaは柔軟なポート転送をサポートしており、Workspaceで実行されているサービスにローカルでアクセスしたり、共有可能なパブリックURL経由でアクセスしたりできます。
ポート転送により、Workspaceでプロジェクトを作業し、外部でコードをテストできます。

### ポートにローカルでアクセスする

Workspaceで実行されているサービスにローカルマシンでアクセスできます。
...
この機能を使用することで、あなたのデータがDaytonaの[プライバシーポリシー](https://www.daytona.io/company/privacy-policy)および[利用規約](https://www.daytona.io/company/terms-of-service)に従って処理されることを承認したものとみなされます。
:::
...
このURLは他の人と共有して、あなたのWorkspace内の指定されたポートで実行されているサービスへのリアルタイムアクセスを提供できます。
