...
Automatic Builderを使用するには、[Workspace作成](/docs/ja/usage/workspaces#create-a-workspace)時に`--builder`フラグの値を`auto`に設定します。このフラグは単一のProjectでWorkspaceを作成する場合にのみ適用されます。
...
Dev Container Builderを使用するには、[Workspace creation](/docs/ja/usage/workspaces#create-a-workspace)中に`--devcontainer-path`フラグをリポジトリ内のDev Container設定パスに設定します。このフラグは、単一のProjectでWorkspaceを作成する場合にのみ適用されます。
...
Daytona Builderによってビルドされたイメージに対して、カスタムビルドレジストリを設定できます。
...
7. 設定を保存するため、コマンドが終了するまで<Keyboard shortcuts="Enter" />を押します。
