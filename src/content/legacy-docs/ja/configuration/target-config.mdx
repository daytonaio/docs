...
Target Configurationとは、さまざまな[Providers](/docs/configuration/providers)によって促進される開発セットアップがデプロイされ、管理される特定の宛先または環境を指します。Providersは環境を作成するために使用される方法と技術を定義し、Target Configurationsはこれらの環境が存在する正確な場所またはプラットフォームを指定します。
...
Daytonaでは、Target Configurationを削除するオプションを提供しており、不要になったものを管理・削除することができます。Target Configurationが削除されると、そのTarget Configuration上でWorkspaceを管理またはデプロイすることはできなくなります。
...
2. 削除したいConfigurationを選択して**`Enter`**を押します。
...
:::note
以前に[Runner](/docs/usage/runners)をインストールしていない場合、DaytonaはProviderを管理するRunnerの選択を求めます。選択すると、DaytonaはターゲットシステムにRunnerをインストールし、Daytona Serverに接続します。
:::
...
:::note
以前に[Runner](/docs/usage/runners)をインストールしていない場合、DaytonaはProviderを管理するRunnerを選択するよう促します。選択すると、DaytonaはターゲットシステムにRunnerをインストールし、Daytona Serverに接続します。
:::
...
:::note
以前に [Runner](/docs/usage/runners) をインストールしていない場合、Daytona は Provider を管理する Runner を選択するよう促します。選択すると、Daytona はターゲットシステムに Runner をインストールし、Daytona Server に接続します。
:::
