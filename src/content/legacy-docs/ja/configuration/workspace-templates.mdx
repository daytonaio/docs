...
description: 複数のWorkspace間でWorkspace Templatesを管理・再利用する方法を学びます。
---
...
Workspace Templateは開発環境の管理を簡素化するように設計されており、新しいWorkspaceに簡単に適用できる設定を事前定義することができます。

Workspace TemplateにはWorkspace内でWorkspaceを定義・管理するために必要なすべてのプロパティが含まれています。リポジトリURL、ビルド設定、環境変数などの情報を保存します。これにより、以前に設定したテンプレートを簡単に再利用でき、複数のWorkspace間で一貫性のある再現可能なセットアップを確保できます。
...
- **Build Configuration**
   ...
   [Automatic](/docs/ja/usage/builders#automatic)はWorkspaceに最も適切なBuilderを自動検出します。
...
このコマンドを実行すると、DaytonaはWorkspace Templatesのリストを表示します。
