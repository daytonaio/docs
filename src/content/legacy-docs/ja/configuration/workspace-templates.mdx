...
Workspace TemplateにはWorkspace内でWorkspaceを定義し管理するために必要なすべてのプロパティが含まれています。リポジトリURL、ビルド設定、環境変数などの情報を保存します。これにより、以前に設定したテンプレートを簡単に再利用でき、複数のWorkspace間で一貫性があり再現可能なセットアップを確保できます。

Workspace TemplateはWorkspaceをセットアップするためのすべての重要な要素をカプセル化します：
...
- **Build Configuration**
   ...
   [Automatic](/docs/usage/builders#automatic)はWorkspaceに最も適切なBuilderを自動検出します。

   [Devcontainer](/docs/usage/builders#dev-container)は`devcontainer.json`ファイルで指定された事前定義された開発コンテナを利用します。

   [Custom image](/docs/usage/builders#custom-image)はカスタムベースコンテナを指定してWorkspaceイメージをビルドします。

   [None](/docs/usage/builders#none)はデフォルトのベースイメージ（`daytonaio/workspace-project`）を使用してWorkspaceをビルドします。
...
Daytonaは、Workspace Templateを追加するオプションを提供しており、複数のWorkspace間でWorkspaceプロパティを保存して再利用することができます。
...
- **サンプルから作成**
   ...
   Gitリポジトリの準備ができていない場合は、Daytonaによって提供されるサンプルプロジェクトからWorkspace Templateを作成できます。
...
Daytonaは、Workspace Templateをエクスポートするオプションを提供しており、Workspace TemplateのプロパティをJSONオブジェクトとして保存することができます。
