...
- `Failed to connect to server: Get "http://server/health": lookup server...`エラーによりWorkspaceの作成に失敗する。
- Tailscale接続の確立時にタイムアウトが発生し、`daytona code`または`daytona ssh`を使用してWorkspaceに入ることができない。
...
WSL2でDaytonaを実行すると、Visual Studio Code IDEとシームレスに動作しません。この問題は、Visual Studio CodeがネイティブのWindows SSHクライアントに依存している一方で、Daytonaがワークスペース固有のSSH接続設定を構成するためにLinux SSH設定ファイルを変更することから生じます。
...
この問題はDaytona固有のものではなく、Visual Studio Codeリモート開発プラグインの制限に起因します。このプラグインは現在、`code` CLIを呼び出す際にカスタムの「remote SSH path」フラグを渡すことをサポートしていません。この機能がないため、DaytonaのLinux固有の設定への依存がVisual Studio CodeのWindows ネイティブツールへの依存と競合します。
