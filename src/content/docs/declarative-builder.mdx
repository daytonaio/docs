...
2. **Pre-built Images**: Create and register reusable images that can be shared across multiple Sandboxes

It provides the following capabilities. For a complete API reference and method signatures, check the [Python](/docs/python-sdk) and [TypeScript](/docs/typescript-sdk) SDK references.
...
For detailed method signatures and usage examples, refer to the [Python](/docs/python-sdk) and [TypeScript](/docs/typescript-sdk) SDK references.
...
{
    timeout: 0,
    onImageBuildLogs: (msg) => process.stdout.write(msg),
}
)
```
...
daytona.create_image(image_name, image, on_logs=lambda log_line: print(log_line, end=''))
...
await daytona.createImage(imageName, image, { onLogs: (msg) => process.stdout.write(msg) })
...
:::note
If your Dockerfile relies on specific context files or directories, you can use the `add_local_file` and `add_local_directory` methods to add them to the image.
This way, the client uploads the contents onto a object storage managed by Daytona which will in turn be added to the build context.
:::
