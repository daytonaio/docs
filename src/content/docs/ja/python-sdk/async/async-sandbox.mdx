...
- `user` _str_ - Sandbox内で実行されるOSユーザー。
- `env` _Dict[str, str]_ - Sandbox内で設定される環境変数。
- `labels` _Dict[str, str]_ - Sandboxに付加されるカスタムラベル。
- `public` _bool_ - Sandboxが公開アクセス可能かどうか。
- `target` _str_ - Sandboxが実行されるランナーのターゲット場所。
- `cpu` _int_ - Sandboxに割り当てられるCPU数。
- `gpu` _int_ - Sandboxに割り当てられるGPU数。
- `memory` _int_ - Sandboxに割り当てられるメモリ量（GiB単位）。
- `disk` _int_ - Sandboxに割り当てられるディスク容量（GiB単位）。
...
- `path_to_project` _str_ - プロジェクトルートディレクトリへのパス。相対パスはユーザーのルートディレクトリを基準に解決されます。
...
Sandboxが'started'状態に達するまで待機します。Sandboxのステータスを'started'状態に達するまで、エラーが発生するまで、またはタイムアウトするまでポーリングします。
...
Sandboxが'stopped'状態に達するまで待機します。Sandboxのステータスを'stopped'状態に達するまで、エラーが発生するまで、またはタイムアウトするまでポーリングします。Sandboxが停止するまで最大60秒間待機します。
