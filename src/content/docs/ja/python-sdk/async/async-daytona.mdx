...
リソースリークを回避するために、このメソッドを呼び出す必要があります。
...
指定されたレジストリで利用可能なイメージまたは宣言的なDaytonaイメージからSandboxを作成します。
...
- `timeout` _Optional[float]_ - Sandbox作成のタイムアウト（秒）。0は無制限を意味します。
  デフォルトは60秒です。
...
- `timeout` _Optional[float]_ - Sandbox削除のタイムアウト（秒）。0は無制限を意味します。
  デフォルトは60秒です。
  
**例外**:

- `DaytonaError` - Sandboxの削除に失敗またはタイムアウトした場合
...
- `target` _Optional[SandboxTargetRegion]_ - Sandboxのターゲット環境。ここまたは環境変数`DAYTONA_TARGET`で設定されていない場合、デフォルトで`'us'`になります。
