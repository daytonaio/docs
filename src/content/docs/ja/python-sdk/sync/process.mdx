...
Sandbox 内でシェルコマンドを実行します。

**引数**:

- `command` _str_ - 実行するシェルコマンド。
- `cwd` _Optional[str]_ - コマンド実行時の作業ディレクトリ。指定しない場合は Sandbox のルートディレクトリが使用されます。デフォルトはユーザーのルートディレクトリです。
- `env` _Optional[Dict[str, str]]_ - コマンド用に設定する環境変数。
- `timeout` _Optional[int]_ - コマンドの完了を待つ最大秒数。0 の場合は無制限に待機します。
...
- `ExecuteResponse` - コマンド実行結果を含みます:
  ...
  - result: コマンドの標準出力
  - artifacts: `stdout`（result と同じ）および `charts`（matplotlib チャートのメタデータ）を含む ExecutionArtifacts オブジェクト
...
適切な言語ランタイムを使用して Sandbox 内でコードを実行します。

**引数**:

- `code` _str_ - 実行するコード。
- `params` _Optional[CodeRunParams]_ - コード実行用のパラメータ。
- `timeout` _Optional[int]_ - コードの完了を待つ最大秒数。0 の場合は無制限に待機します。
...
```
  ...
  Matplotlib のチャートは自動的に検出され、`ExecutionArtifacts` オブジェクトの `charts` フィールドで返されます。
...
Sandbox で新しい長時間実行されるバックグラウンドセッションを作成します。

セッションはコマンド間で状態を保持するバックグラウンドプロセスであり、複数の関連コマンドや永続的な環境セットアップが必要なシナリオに最適です。長時間実行されるコマンドを実行し、プロセスのステータスを監視できます。

**引数**:

- `session_id` _str_ - 新しいセッションの一意な識別子。
...
セッション内で実行されたコマンドのログを取得します。セッション内で実行されたコマンドからの完全な出力（stdout および stderr）を取得します。
...
- `str` - stdout と stderr の両方を含むコマンドの完全な出力。
...
セッションで実行されたコマンドのログを、利用可能になり次第、非同期で取得および処理します。
...
Sandboxからセッションを終了および削除し、それに関連するリソースをクリーンアップします。
...
コマンド実行から得られるアーティファクト。

**属性**:

- `stdout` _str_ - コマンドの標準出力。`ExecuteResponse` の `result` と同じ
- `charts` _Optional[List[Chart]]_ - matplotlib からのチャートメタデータのリスト
