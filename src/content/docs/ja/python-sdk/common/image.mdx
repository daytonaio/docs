...
Daytona サンドボックスのためのイメージ定義を表します。
このクラスを直接構築しないでください。代わりに、`Image.base()`、`Image.debian_slim()`、`Image.from_dockerfile()` などの静的ファクトリメソッドのいずれかを使用してください。
...
- `extra_options` - str = "": pip に渡す追加オプション。指定された文字列は
...
- `optional_dependencies` - list[str] = []: pyproject.toml ファイルからインストールするオプション依存関係。
