...
Daytona SDKは、Daytonaとの相互作用のための公式の[Python](/docs/python-sdk)および[TypeScript](/docs/typescript-sdk)インターフェースを提供し、開発環境をプログラム的に管理し、コードを実行することを可能にします。[Python SDK](/docs/python-sdk)は同期と非同期の両方のプログラミングモデルをサポートしており、非同期クラスには`Async`のプレフィックスが付きます。

AI AgentのためのあなたのはじめてのDaytona Sandboxを作成し実行するためのステップバイステップガイドに従ってください。

環境変数の設定やステージング環境での実験的機能へのアクセスを含む追加設定の手順については、[Configuration](/docs/configuration)をご覧ください。
...
このエンドポイントにプログラムでアクセスする必要がありますか？[Preview & Authentication](/docs/preview-and-authentication)について詳しく学んでください。

:::tip
Sandbox [Web Terminal](/docs/web-terminal)には、ポート`22222`のプレビューURLを出力するか、単にDashboard -> Sandboxesに移動してTerminal入力サインをクリックすることでアクセスできます。
:::

## LLMに接続する

以下のスニペットは、Anthropic APIを使用してLLMに接続し、Claudeに25の階乗を取得するコードを生成するよう依頼し、それをDaytona Sandbox内で実行します：
...
[ローカルデバイスからのイメージ](/docs/snapshots#using-a-local-image)を使用したい場合や、コマンドラインインターフェースを使用してSandboxを管理することを好む場合は、以下を実行してDaytona CLIをインストールしてください：
