...
ファイルが閉じられたことをlanguage serverに通知します。エディタでファイルが閉じられた際に呼び出すべきで、
language serverがそのファイルに関連するリソースをクリーンアップできるようにします。
...
ファイルが開かれたことをlanguage serverに通知し、そのファイルに対する
...
- `query` _string_ - シンボル名に対してマッチングする検索クエリ
...
言語サーバーを停止します。LSPサーバーが不要になった際にシステムリソースを
解放するために呼び出す必要があります。
