...
コードインテリジェンス機能を提供するLanguage Server Protocol機能を提供し、
コード補完、シンボル検索などのIDE風の機能を実現します。
...
ファイルが閉じられたことを言語サーバーに通知します。エディタでファイルが閉じられた際に呼び出すべきで、
言語サーバーがそのファイルに関連するリソースをクリーンアップできるようにします。
...
ファイルが開かれたことを言語サーバーに通知し、そのファイルに対する
診断や補完などの言語機能を有効にします。サーバーは
...
- `Promise<LspSymbol[]>` - 全ファイルからの一致するシンボルのリスト。各シンボルには以下が含まれます：
  - name: シンボルの名前
  - kind: シンボルの種類（関数、クラス、変数など）
  - location: ファイル内のシンボルの位置
...
言語サーバーを停止します。LSPサーバーが不要になった際に、
システムリソースを解放するために呼び出す必要があります。
...
- `Promise<LspSymbol[]>` - 全ファイルから一致するシンボルのリスト。各シンボルには以下が含まれます:
  - name: シンボルの名前
  - kind: シンボルの種類（関数、クラス、変数など）
  - location: ファイル内のシンボルの位置
