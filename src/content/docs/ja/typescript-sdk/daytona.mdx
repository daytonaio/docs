...
- `timeout?` _number_ - オプションのタイムアウト（秒）（0は無制限を意味します）
...
- `user?` _string_ - Sandboxに使用するオプションのosユーザー
...
- `apiKey?` _string_ - Daytona APIでの認証用のAPIキー
- `apiUrl?` _string_ - Daytona APIのURL。ここで設定されておらず、環境変数DAYTONA_API_URLでも設定されていない場合、デフォルトで'https://app.daytona.io/api'になります。
- `jwtToken?` _string_ - Daytona APIでの認証用のJWTトークン。設定されていない場合は、環境変数`DAYTONA_JWT_TOKEN`で提供するか、代わりにAPIキーを提供する必要があります。
- `organizationId?` _string_ - JWTベース認証で使用される組織ID。JWTトークンが提供される場合は必須で、ここまたは環境変数`DAYTONA_ORGANIZATION_ID`で設定する必要があります。
- ~~`serverUrl?`~~ _string_ - **_非推奨_** - 代わりに`apiUrl`を使用してください。このプロパティは将来のバージョンで削除される予定です。
- `target?` _string_ - Sandboxesのターゲット場所
...
- `cpu?` _number_ - Sandboxのコア数でのCPU割り当て
- `disk?` _number_ - SandboxのGiB単位でのディスク容量割り当て
- `gpu?` _number_ - Sandboxの単位でのGPU割り当て
