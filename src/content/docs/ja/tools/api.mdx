...
| **`labels`** | query | false | undefined | フィルタリングに使用するJSON形式のラベル |
...
サンドボックス内の複数ファイルでテキスト/パターンを置換する
...
| 名前 | 場所 | 必須 | 型 | 説明 |
| :--- | :------- | :------- | :--- | :---------- |
| **`X-Daytona-Organization-ID`** | header | false | undefined | JWTと組み合わせて組織IDを指定するために使用 |
| **`sandboxId`** | path | true | undefined | undefined |
...
gitリポジトリでブランチまたはコミットをチェックアウトする
...
| Name | Location | Required | Type | Description |
| :--- | :------- | :------- | :--- | :---------- |
| **`X-Daytona-Organization-ID`** | header | false | undefined | 組織IDを指定するためにJWTと併用します |
| **`sandboxId`** | path | true | undefined | undefined |
...
サンドボックスプロジェクト内でLspサーバープロセスを開始します
...
サンドボックスプロジェクト内のLspサーバープロセスを停止します
