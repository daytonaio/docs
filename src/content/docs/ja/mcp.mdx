...
- **Sandboxの作成**
  - 新しいDaytona sandboxを作成します
  - パラメータ:
    - `target` (デフォルト: "us"): ターゲット地域
    - `snapshot`: sandboxのスナップショット（オプション）
    - `auto_stop_interval` (デフォルト: "15"): 自動停止間隔（分単位、0で無効化）
    - `auto_archive_interval` (デフォルト: "10080"): 自動アーカイブ間隔（分単位、0で最大間隔が使用される）

- **Sandboxの削除**
  - 既存のDaytona sandboxを削除します

### ファイル操作

- **ファイルのダウンロード**
  - Daytona sandboxからファイルをダウンロードします
  - テキストまたはbase64エンコードされた画像としてコンテンツを返します
  - パラメータ:
    - `file_path`: ファイルへのパス

- **ファイルのアップロード**
  - Daytona sandboxにファイルをアップロードします
