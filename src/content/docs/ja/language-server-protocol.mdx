...
Daytona SDKは、PythonとTypeScriptを使用してLSPサーバーを作成するオプションを提供します。`path_to_project`の開始点は、デフォルトで現在のSandboxユーザーのルートになります - 例えば、`workspace/project`は`/home/[username]/workspace/project`を意味しますが、`/`で始めることで絶対パスを定義することもできます。
