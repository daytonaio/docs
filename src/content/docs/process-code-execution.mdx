...
Daytona SDK provides an option to execute shell commands and manage background processes in Sandboxes. The workdir for executing defaults to the current Sandbox user's root - e.g. `/workspace/repo` implies `/home/[username]/workspace/repo`, but you can override it with an absolute path.
