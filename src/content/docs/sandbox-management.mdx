...
By default, Sandboxes will never be automatically deleted. However, this can be changed by setting the auto-delete interval. This interval is measured in minutes and it determines how long it takes for a Sandbox to be deleted after it has been continuously stopped. Setting the interval to `0` will disable auto-deletion.
