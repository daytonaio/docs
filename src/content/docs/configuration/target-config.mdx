...
:::note
If you have not previously installed a Runner, Daytona will prompt you select a Runner to manage the Provider. Upon selecting, Daytona will install the Runner on the target system and connect it to the Daytona Server.
:::
...
4. Enter the appropriate configuration options when prompted. The configuration options vary based on the selected Provider. The following example shows adding a [local Docker Target Configuration](#docker-local).
...
Daytona allows you to create a remote Docker Target Configuration to use for deploying and managing Workspaces. The remote Docker Target Configuration enables you to deploy Workspaces on a remote machine using Docker.
...
```text
Target Config <target_config_name> created set successfully
```
...
```text
Target config <target_config_name> successfully
```
