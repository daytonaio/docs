---
title: Linux/macOS Uninstallation
---
import Aside from "@components/Aside.astro";
export const partial = true;

You can uninstall Daytona after using the official installation script.

<Aside type="caution">
This procedure is destructive and irreversible.
Ensure you have a back up of any Workspace data and relevant configuration before proceeding.
</Aside>

__Prerequisites__

* A POSIX-compatible shell such as bash or zsh.

__Procedure__

1. Run the following command to stop the Daytona Server(if it is running):

    ```shell
    daytona server stop
   ```

2. Run the following commands to purge local Daytona data and remove the binary:

    ```shell
    daytona purge
    sudo rm $(where daytona)
    ```
