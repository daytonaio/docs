...
3.  Enter a name for your Workspace.
    <Aside type="note">
    You can press `F10` to configure advanced settings for your Workspace, including setting the [Builder](../usage/builders).
    </Aside>
...
## Delete Workspaces
You can delete 1 or more Workspaces interactively.
...
2.  Mark Workspaces for deletion by highlighting them and pressing `x`.
    <Aside type="note">
    Pressing `Enter` without using `x` to select individual Workspaces will select the currently highlighted Workspace for deletion.
    </Aside>
3.  Press `Enter`.
3.  Confirm the action.

### Non-Interactively
You can delete 1 or more existing Workspaces non-interactively.
...
__Procedure__
1.  Run the following command where `WORKSPACE` is a space-separated list of 1 or more Workspace names:
    ```shell
    ...
    __Example__
    ```shell
    daytona delete web-microservices erlang-test
    ```
