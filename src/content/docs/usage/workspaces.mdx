...
1. Run the following command to start the guided Workspace creation process with multiple projects:

```shell
daytona create --multi-project
```

2. To create a Daytona Workspace with multiple projects, select one of the three provided options:
...
After adding all the projects, you will be prompted with a summary of the Workspace configuration.
...
6. Wait while Daytona sets up your Workspace. It handles all the initialization and configuration of your environment.
...
Daytona supports port forwarding, allowing you to access services running in a Workspace via a shareable public URL.
With port forwarding, you can work on your [Projects](/docs/usage/projects) in a Workspace and test your code externally.
...
**Example:** Generate a public URL to access port `4321` of the `example-dev-env` Workspace.

```shell
daytona forward 4321 example-dev-env
```

Upon running this command, Daytona will establish a public port forwarding connection and display a message indicating the public port where the forwarded connection is available.

```text
Port available at https://4321-ABC1DeF2GhI3JkLmNoP4RSTuVZ5.try-eu.daytona.app
```
