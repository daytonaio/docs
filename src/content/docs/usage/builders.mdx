...
Development Containers (Dev Containers) provide a consistent and fully featured development environments for your [Project](/docs/usage/projects). A Dev Container defines all the necessary tools, runtime, and environment configurations required to build and run your Project.
...
The `devcontainer.json` file is a JSON file that consists of several components that define the development environment. The file includes the following properties:
