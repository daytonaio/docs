...
Once deployed, the Runner starts up and connects to the [Daytona Server](/docs/usage/server), at which point its status is marked as `Active`.
...
The Runner will connect to the Daytona Server and its status will be marked as `Active`.
